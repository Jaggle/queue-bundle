...
use Jaggle\QueueBundle\Support\Str;
...
class RedisQueue extends Queue implements QueueContract
{
    ...
    {
        return $this->pushRaw($this->createPayload($job));
    }
    ...
    public function pushRaw($payload, $queue = null, array $options = [])
    {
        ...
        $this->getRedis()->rpush($this->getQueue($queue), $payload);
    ...
    public function later($delay, $job, $data = '', $queue = null)
    {
        ...
        $this->getRedis()->zadd($this->getQueue($queue).':delayed', [$this->getTime() + $delay => $payload]);
    ...
    public function pop()
    {
        ...
        $job = $this->getRedis()->lpop($queueName);

        if (! is_null($job)) {
            $this->getRedis()->zadd($queueName.':reserved', [$job => $this->getTime() + $this->expire]);

            $job = json_decode($job);

            $handler = $job->handler;

            return new $handler($job->id, (array) $job->data);
        }
    ...
    {
        $this->getRedis()->zrem($this->getQueue($queue).':reserved', $job);
    }
    ...
    public function migrateExpiredJobs($from, $to)
    {
        ...
        $this->getRedis()->transaction($options, function ($transaction) use ($from, $to) {
    ...
    {
        return (array) $transaction->zrangebyscore($from, '-inf', $time);
    }
     ...
     * @return string
     */
    protected function createPayload(JobContract $job, $data = '', $queue = null)
    {
        $payload = parent::createPayload($job);

        $payload = $this->setMeta($payload, 'uniqueId', $this->getRandomId());
        $payload = $this->setMeta($payload, 'handler', get_class($job));
    ...
    public function setRedis(ClientInterface $client)
    {
